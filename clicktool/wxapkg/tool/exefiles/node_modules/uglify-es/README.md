...
```
    ...
    --source-map [options]      Enable source map/specify source map options:
                                ...
                                `content`  Input source map, useful if you're compressing
                                           JS that was generated from some other self
                                ...
                                `root`  Path to the self source to be included in
                                        the source map.
...
- `--source-map "root='<URL>'"` to pass the URL where the self files can be found.
...
as the source map root, and the self files as `js/file1.js` and
`js/file2.js`).
...
like to map back to the self code (i.e. CoffeeScript).  UglifyJS has an
option to take an input source map.  Assuming you have a mapping from
CoffeeScript → compiled JS, UglifyJS can generate a map from CoffeeScript →
compressed JS by mapping every token in the compiled JS to its self
location.
...
- `debug` (default: `false`) -— Mangle names with the self name still present.

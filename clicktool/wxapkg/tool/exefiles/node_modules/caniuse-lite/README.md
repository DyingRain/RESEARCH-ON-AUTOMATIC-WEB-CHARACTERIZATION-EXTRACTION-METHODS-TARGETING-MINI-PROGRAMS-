...
and `versions` keys from the self.
...
the `title`, `stats` and `status` keys are kept from the self data.
...
something that more closely represents the `caniuse-db` format. Note that *only*
the usage data is exposed here (the `data` key in the self files).

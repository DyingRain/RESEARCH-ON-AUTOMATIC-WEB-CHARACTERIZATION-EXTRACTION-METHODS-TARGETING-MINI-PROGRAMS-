...
def match_tabbar(tabbar_info, text_list):
    ...
    tabbar_prompt = """
    - Role: 小程序页面匹配专家
    - Background: 用户需要根据提供的tabbar名称列表以及其触发的子页面列表和手机当前小程序页面的xml文件内容列表，确定当前页面对应的tabbar。用户已经提供了三个列表，需要通过内容匹配来找到对应的tabbar。
    ...
    - Constrains: 仅基于提供的三个列表进行分析，不引入任何外部数据或假设。
    ...
    - Exampl:
      ...
      Answer: 根据提供的tabbar名称列表和页面内容列表，当前页面对应的tabbar名称应该是'pages/tabmine/tabmine'。原因是页面内容列表中包含了'账户'、'卡券'和'地址管理'等关键词，这些与tabbar名称'pages/tabmine/tabmine'中的'account/MyCashPage'、'account/MyCouponPage'和'account/ShippingAddressPage'相对应，符合页面内容与tabbar名称的关联。
      -例2:
      ...
      Answer: 根据提供的tabbar名称列表和页面内容列表，当前页面对应的tabbar名称应该是'pages/index/index'。原因是页面内容列表中包含了'推荐'、'鞋类'、'潮服'等关键词，这些与tabbar名称'pages/index/index'中的'pages/index/index'相对应，符合页面内容与tabbar名称的关联。
      -例3:
      ...
      Answer: 根据提供的tabbar名称列表和页面内容列表，当前页面对应的tabbar名称应该是'pages/identify/index'。原因是页面内容列表中包含了'AJ球鞋鉴别'、'COACH鉴别'、'在线鉴别师'等关键词，这些与tabbar名称'pages/identify/index'中的'identify/pages/classificationV2/index'、'identify/pages/web/web'和'identify/pages/publish/index'相对应，符合页面内容与tabbar名称的关联。

    - Initialization: 在第一次对话中，请直接输出以下问题的答案并给出原因："""
    print(llm_api.invoke(tabbar_prompt + f"tabbar名称列表：{tabbar_info}.页面文本列表：{text_list}").content)
...
def page_brief(page_text_list):
    ...
    """
    result = llm_api.invoke(prompt + f"页面文本内容：{page_text_list}。").content
    return result
...
def judge_clickable(page_description, ui_text, ):
    ...
    - OutputFormat: JSON格式，包含一个键`result`，其值为`"yes"`（会触发页面跳转）或`"no"`（不会触发页面跳转）。
    - Workflow:
    ...
    - Examples:
      ...
      - 例子3：
        ...
        - UI组件文本内容：“点赞”
        - 结果：`{"result": "no"}`
    - Initialization: 在第一次对话中，请直接输出以下问题的结果：
    """
    result = llm_api.invoke(prompt + f"当前页面描述：{page_description}。当前ui文本内容：{ui_text}").content
    return result
